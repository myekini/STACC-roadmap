...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn('fixed inset-0 z-40 bg-on-surface/25 backdrop-blur-[2px] data-[state=open]:animate-in data-[state=closed]:animate-out', className)}
    {...props}
  />
));
SheetOverlay.displayName = DialogPrimitive.Overlay.displayName;

const sheetVariants = cva('fixed z-50 flex flex-col border-outline-variant bg-surface transition ease-in-out', {
  variants: {
    side: {
      right: 'inset-y-0 right-0 h-full w-full max-w-[480px] border-l shadow-[-20px_0_60px_rgba(0,0,0,0.5)]',
      left: 'inset-y-0 left-0 h-full w-3/4 max-w-sm border-r shadow-[20px_0_60px_rgba(0,0,0,0.5)]',
      top: 'inset-x-0 top-0 border-b shadow-[0_20px_60px_rgba(0,0,0,0.5)]',
      bottom: 'inset-x-0 bottom-0 border-t shadow-[0_-20px_60px_rgba(0,0,0,0.5)]',
    },
  },
