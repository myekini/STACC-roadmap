...
insert into public.nodes (slug, path_id, name, subtitle, description, icon, "order", est_hours, xp_reward, skills) values
  ...
  ('found-python',   'foundations', 'Python Basics', 'Variables to pandas', 'Write scripts that load, clean, and reshape data. The working language of every path that follows.', 'code', 1, 12, 100, '{Python syntax,Data types,Functions,Pandas intro}'),
  ...
  ('de-dbt',           'de', 'dbt', 'Data build tool', 'Transformations as code: models, tests, docs, and environments with dbt.', 'code_blocks', 3, 12, 200, '{Models,Tests,Jinja,Environments}'),
  ...
  ('de-cloud',         'de', 'Cloud Platforms', 'AWS / GCP', 'Object storage, warehouses, IAM, and the managed services data teams actually use.', 'deployed_code', 5, 14, 200, '{S3/GCS,BigQuery/Redshift,IAM,Cost basics}'),
  ('de-spark',         'de', 'Spark — Advanced', 'Distributed compute', 'Partitioning, shuffles, and writing PySpark that scales past a single machine.', 'memory', 6, 16, 250, '{PySpark,Partitioning,Shuffles,Tuning}'),
  ...
  -- DATA ANALYSIS
  ('da-eda',           'da', 'Exploratory Data Analysis', 'Interrogate the data', 'Profile datasets, find outliers and patterns, and form hypotheses worth testing.', 'find_in_page', 1, 10, 150, '{Profiling,Outliers,Univariate/bivariate analysis}'),
  ('da-visualization', 'da', 'Data Visualization', 'Matplotlib, Seaborn', 'Choose the right chart, encode honestly, and build plots people actually read.', 'bar_chart', 2, 10, 150, '{Chart choice,Matplotlib,Seaborn,Perception}'),
  ('da-dashboards',    'da', 'Dashboard Design', 'Interfaces for decisions', 'Layout, hierarchy, and interactivity for dashboards that answer questions at a glance.', 'dashboard', 3, 10, 150, '{Layout,KPI design,Filters,Performance}'),
  ('da-storytelling',  'da', 'Data Storytelling', 'Insight to action', 'Structure findings as narratives that move stakeholders to a decision.', 'edit_note', 4, 8, 150, '{Narrative structure,Executive summaries,Presenting}'),
  ...
  -- DATA SCIENCE
  ('ds-ml',            'ds', 'ML Fundamentals', 'Supervised learning core', 'Regression, classification, overfitting, and the bias-variance tradeoff in practice.', 'model_training', 1, 14, 200, '{Regression,Classification,Bias-variance,scikit-learn}'),
  ('ds-features',      'ds', 'Feature Engineering', 'Signal from raw data', 'Encodings, scaling, leakage traps, and features that actually move metrics.', 'settings_input_component', 2, 10, 150, '{Encodings,Scaling,Leakage,Selection}'),
  ...
  ('ds-experiments',   'ds', 'Experimentation & A/B Testing', 'Causal by design', 'Design experiments, size samples, and read results without fooling yourself.', 'biotech', 4, 12, 200, '{Experiment design,Power,Significance,Pitfalls}'),
  ('ds-deployment',    'ds', 'Model Deployment', 'Models as services', 'Package and serve models behind APIs with versioning and rollback.', 'publish', 5, 12, 200, '{FastAPI,Serialization,Versioning,Rollback}'),
  ...
  ('ds-llm',           'ds', 'LLM Fine-tuning & RAG', 'Adapt foundation models', 'Fine-tuning versus retrieval, dataset curation, and evaluating LLM output.', 'smart_toy', 7, 16, 250, '{Fine-tuning,RAG,Dataset curation,Evals}'),

  -- AI ENGINEERING (unlocks after DE + DS)
  ('ai-llm-apis',      'ai-engineering', 'LLM APIs & Orchestration', 'OpenAI, Anthropic, Gemini', 'Structured outputs, tool use, streaming, and orchestrating multi-step LLM calls.', 'smart_toy', 1, 12, 250, '{Tool use,Structured output,Streaming,Orchestration}'),
  ('ai-rag',           'ai-engineering', 'RAG System Design', 'Retrieval done right', 'Chunking, hybrid search, reranking, and grounding answers in your own data.', 'find_in_page', 2, 14, 250, '{Chunking,Hybrid search,Reranking,Grounding}'),
  ('ai-agents',        'ai-engineering', 'AI Agents & Tool Use', 'Systems that act', 'Agent loops, tool design, guardrails, and when not to build an agent.', 'smart_toy', 3, 14, 250, '{Agent loops,Tool design,Guardrails,Memory}'),
  ...
  ('ai-llmops',        'ai-engineering', 'LLMOps & Evaluation', 'Measure or guess', 'Eval suites, regression testing prompts, observability, and cost control.', 'analytics', 5, 12, 250, '{Evals,Prompt regression,Tracing,Cost control}'),
  ('ai-product',       'ai-engineering', 'AI Product Design', 'Architecture end-to-end', 'Design a full AI product: latency budgets, fallbacks, UX for uncertainty.', 'explore', 6, 14, 300, '{Latency budgets,Fallbacks,UX for AI,Architecture}'),
  ...
  ('ml-cicd',          'mlops', 'CI/CD for ML', 'Automate the path to prod', 'Pipelines that test data, code, and models before anything ships.', 'published_with_changes', 2, 12, 200, '{GitHub Actions,Model tests,Artifacts,Environments}'),
  ('ml-monitoring',    'mlops', 'Monitoring & Drift', 'Know when models rot', 'Data drift, concept drift, and alerting on the metrics that predict failure.', 'analytics', 3, 12, 250, '{Data drift,Concept drift,Alerting,Dashboards}'),
  ...
  ('ml-platform',      'mlops', 'ML Platform Design', 'End-to-end ownership', 'Design the platform: from experiment tracking to deployment paths for a whole team.', 'schema', 5, 16, 300, '{Experiment tracking,Registries,Platform architecture}');
