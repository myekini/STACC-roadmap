...
from (values
  ('found-python', 'Work through the core Python course modules', 'watch', 1, null, null),
  ('found-python', 'Build: clean a messy CSV with pandas and export a tidy dataset', 'build', 2, null, null),
  ...
  ('found-sql', 'Complete the interactive SQL lessons', 'read', 1, null, null),
  ('found-sql', 'Build: answer 5 business questions against a sample database', 'build', 2, null, null),
  ...
  ('found-git', 'Read Pro Git chapters 1–3', 'read', 1, null, null),
  ('found-git', 'Build: open a pull request with a reviewed change on your own repo', 'build', 2, null, null),
  ...
  ('found-cli', 'Watch the Missing Semester shell lecture', 'watch', 1, null, null),
  ('found-cli', 'Build: write a shell script that organizes files by extension', 'build', 2, null, null),
  ...
  ('found-stats', 'Explore distributions and inference on Seeing Theory', 'read', 1, null, null),
  ('found-stats', 'Build: analyze a dataset and report mean/median skew with plots', 'build', 2, null, null),
  ...
  ('found-ai', 'Read the prompt engineering fundamentals guide', 'read', 1, null, null),
  ('found-ai', 'Build: solve a coding task end-to-end with an AI assistant, documenting prompts', 'build', 2, null, null),
  ...
  ('de-etl', 'Study the ingestion + ETL weeks of the DE Zoomcamp', 'watch', 1, null, null),
  ('de-etl', 'Build: a pipeline that ingests a public API into a database daily', 'build', 2, null, null),
  ('de-etl', 'Checkpoint quiz', 'quiz', 3, '{"question":"An idempotent pipeline run…","options":["Runs faster each time","Produces the same result if re-run","Never fails","Requires no scheduler"],"correctIndex":1,"explanation":"Idempotency means re-running the same load does not duplicate or corrupt data — key for safe retries and backfills."}', null),
  ('de-modeling', 'Read Kimball''s dimensional modeling essentials', 'read', 1, null, null),
  ('de-modeling', 'Build: design a star schema for an e-commerce domain', 'build', 2, null, null),
  ('de-modeling', 'Checkpoint quiz', 'quiz', 3, '{"question":"In a star schema, facts are…","options":["Descriptive attributes","Measurable events at a declared grain","Lookup tables","Slowly changing dimensions"],"correctIndex":1,"explanation":"Fact tables hold measurable events at a specific grain; dimensions carry the descriptive context."}', null),
  ('de-dbt', 'Complete dbt Fundamentals', 'watch', 1, null, null),
  ('de-dbt', 'Build: a dbt project with staging/marts models and tests', 'build', 2, null, null),
  ('de-dbt', 'Checkpoint quiz', 'quiz', 3, '{"question":"dbt primarily handles which part of ELT?","options":["Extract","Load","Transform","Orchestration"],"correctIndex":2,"explanation":"dbt transforms data already loaded in the warehouse using SQL models with testing and docs."}', null),
  ('de-orchestration', 'Work through Airflow fundamentals', 'watch', 1, null, null),
  ('de-orchestration', 'Build: schedule your ETL pipeline as a DAG with retries and alerts', 'build', 2, null, null),
  ('de-orchestration', 'Checkpoint quiz', 'quiz', 3, '{"question":"A DAG in orchestration is…","options":["A database table","A directed acyclic graph of tasks","A deployment artifact","A data quality rule"],"correctIndex":1,"explanation":"Workflows are modeled as directed acyclic graphs so dependencies run in order without cycles."}', null),
  ('de-cloud', 'Study object storage, warehouse, and IAM basics on one cloud', 'watch', 1, null, null),
  ('de-cloud', 'Build: deploy your pipeline to run on cloud infrastructure', 'build', 2, null, null),
  ...
  ('de-spark', 'Study the Spark SQL programming guide', 'read', 1, null, null),
  ('de-spark', 'Build: process a dataset too large for pandas with PySpark', 'build', 2, null, null),
  ('de-spark', 'Checkpoint quiz', 'quiz', 3, '{"question":"A shuffle in Spark is expensive because…","options":["It uses GPUs","Data moves across the network between partitions","It recompiles the job","It writes to the driver"],"correctIndex":1,"explanation":"Shuffles repartition data across executors over the network — the main cost to minimize in Spark jobs."}', null),
  ('de-streaming', 'Complete Kafka 101', 'watch', 1, null, null),
  ('de-streaming', 'Build: a producer/consumer pair processing events in real time', 'build', 2, null, null),
  ('de-streaming', 'Checkpoint quiz', 'quiz', 3, '{"question":"Consumer groups in Kafka enable…","options":["Message encryption","Parallel consumption with each partition read by one member","Schema validation","Exactly-once storage"],"correctIndex":1,"explanation":"Partitions are divided among group members, giving horizontal scale while preserving per-partition order."}', null),
  ('de-vectordb', 'Read the vector database fundamentals guide', 'read', 1, null, null),
  ('de-vectordb', 'Build: embed a document set and serve similarity search', 'build', 2, null, null),
  ('de-vectordb', 'Checkpoint quiz', 'quiz', 3, '{"question":"Embeddings are…","options":["Compressed files","Dense vectors capturing semantic meaning","Database indexes","Encrypted tokens"],"correctIndex":1,"explanation":"Embeddings map text/images into dense vectors where semantic similarity becomes geometric closeness."}', null),

  ('da-eda', 'Complete the data cleaning course', 'watch', 1, null, null),
  ('da-eda', 'Build: a full EDA notebook on a dataset you have never seen', 'build', 2, null, null),
  ('da-eda', 'Checkpoint quiz', 'quiz', 3, '{"question":"The first thing to check in a new dataset is…","options":["Model accuracy","Missing values, types, and distributions","Dashboard colors","Feature importance"],"correctIndex":1,"explanation":"Profiling — completeness, types, ranges, distributions — comes before any analysis or modeling."}', null),
  ('da-visualization', 'Study chart-choice and perception principles', 'read', 1, null, null),
  ('da-visualization', 'Build: remake three bad charts into honest, readable ones', 'build', 2, null, null),
  ('da-visualization', 'Checkpoint quiz', 'quiz', 3, '{"question":"For comparing quantities across categories, prefer…","options":["Pie chart","Bar chart","3D surface","Word cloud"],"correctIndex":1,"explanation":"Length on a common baseline (bars) is the most accurately perceived encoding for comparisons."}', null),
  ('da-dashboards', 'Read dashboard design best practices', 'read', 1, null, null),
  ('da-dashboards', 'Build: a KPI dashboard answering three stakeholder questions', 'build', 2, null, null),
  ('da-dashboards', 'Checkpoint quiz', 'quiz', 3, '{"question":"A good dashboard leads with…","options":["Every available metric","The most decision-relevant KPIs","Raw tables","Filters"],"correctIndex":1,"explanation":"Hierarchy matters: the questions users came to answer belong at the top, detail below."}', null),
  ('da-storytelling', 'Work through storytelling-with-data exercises', 'read', 1, null, null),
  ('da-storytelling', 'Build: a 5-slide narrative from one of your analyses', 'build', 2, null, null),
  ('da-storytelling', 'Checkpoint quiz', 'quiz', 3, '{"question":"A data story should end with…","options":["The methodology","A recommended action","All caveats","The raw data"],"correctIndex":1,"explanation":"Analyses drive decisions: close with the action the evidence supports, then back it up."}', null),
  ('da-bi', 'Complete a Power BI (or Metabase) learning path', 'watch', 1, null, null),
  ('da-bi', 'Build: publish a governed dashboard with a shared metric definition', 'build', 2, null, null),
  ('da-bi', 'Checkpoint quiz', 'quiz', 3, '{"question":"A semantic layer exists to…","options":["Speed up SQL","Define metrics once and reuse everywhere","Replace the warehouse","Encrypt data"],"correctIndex":1,"explanation":"Semantic layers centralize metric definitions so every tool and team reports the same numbers."}', null),
  ('da-ai-analysis', 'Study prompting patterns for analysis work', 'read', 1, null, null),
  ('da-ai-analysis', 'Build: run an analysis with an LLM assistant and validate every claim', 'build', 2, null, null),
  ('da-ai-analysis', 'Checkpoint quiz', 'quiz', 3, '{"question":"When an LLM produces an analysis claim, you should…","options":["Ship it","Verify it against the data before using it","Ask it to be confident","Lower the temperature"],"correctIndex":1,"explanation":"LLMs speed up analysis but hallucinate; every number and claim must be validated against the source data."}', null),

  ('ds-ml', 'Complete the intro ML course', 'watch', 1, null, null),
  ('ds-ml', 'Build: train and compare two models on a tabular dataset', 'build', 2, null, null),
  ('ds-ml', 'Checkpoint quiz', 'quiz', 3, '{"question":"High training accuracy but poor test accuracy indicates…","options":["Underfitting","Overfitting","Data leakage is impossible","A perfect model"],"correctIndex":1,"explanation":"The model memorized training data instead of generalizing — the classic overfitting signature."}', null),
  ('ds-features', 'Complete the feature engineering course', 'watch', 1, null, null),
  ('ds-features', 'Build: engineer features that measurably beat the raw baseline', 'build', 2, null, null),
  ...
  ('ds-evaluation', 'Study evaluation metrics and cross-validation', 'read', 1, null, null),
  ('ds-evaluation', 'Build: an evaluation report with CV, baselines, and the right metric', 'build', 2, null, null),
  ...
  ('ds-experiments', 'Study experiment design and statistical power', 'read', 1, null, null),
  ('ds-experiments', 'Build: design an A/B test plan with hypothesis, sample size, and decision rule', 'build', 2, null, null),
  ('ds-experiments', 'Checkpoint quiz', 'quiz', 3, '{"question":"Peeking at A/B results daily and stopping early…","options":["Is best practice","Inflates false positives","Reduces variance","Has no effect"],"correctIndex":1,"explanation":"Repeated significance checks without correction dramatically inflate the false-positive rate."}', null),
  ('ds-deployment', 'Study FastAPI model-serving patterns', 'read', 1, null, null),
  ('ds-deployment', 'Build: serve a trained model behind a versioned REST endpoint', 'build', 2, null, null),
  ('ds-deployment', 'Checkpoint quiz', 'quiz', 3, '{"question":"Model artifacts should be…","options":["Retrained per request","Versioned and loaded at startup","Stored in the client","Hardcoded"],"correctIndex":1,"explanation":"Versioned artifacts loaded at startup give reproducible, fast inference and clean rollbacks."}', null),
  ('ds-deeplearning', 'Work through fast.ai practical deep learning', 'watch', 1, null, null),
  ('ds-deeplearning', 'Build: fine-tune a pretrained network on your own image/text data', 'build', 2, null, null),
  ('ds-deeplearning', 'Checkpoint quiz', 'quiz', 3, '{"question":"Transfer learning works because…","options":["Small data trains from scratch","Early layers learn reusable general features","GPUs are fast","Labels are optional"],"correctIndex":1,"explanation":"Pretrained networks capture general features; fine-tuning adapts them to your task with little data."}', null),
  ('ds-llm', 'Complete the Hugging Face NLP course core chapters', 'watch', 1, null, null),
  ('ds-llm', 'Build: compare a RAG baseline vs fine-tuning on one task, with evals', 'build', 2, null, null),
  ...
  ('ai-llm-apis', 'Checkpoint quiz', 'quiz', 3, '{"question":"Tool use lets an LLM…","options":["Train itself","Call functions you define and use the results","Access weights","Skip safety"],"correctIndex":1,"explanation":"You declare tools; the model requests calls with arguments and incorporates your returned results."}', null),
  ...
  ('ai-rag', 'Checkpoint quiz', 'quiz', 3, '{"question":"Reranking improves RAG by…","options":["Making embeddings smaller","Reordering retrieved candidates by relevance before generation","Caching responses","Increasing chunk size"],"correctIndex":1,"explanation":"A reranker scores query-document pairs precisely, promoting the truly relevant chunks into context."}', null),
  ...
  ('ml-docker', 'Checkpoint quiz', 'quiz', 3, '{"question":"Docker image layers are…","options":["Random snapshots","Cached filesystem diffs created per instruction","VM disks","Encrypted volumes"],"correctIndex":1,"explanation":"Each Dockerfile instruction creates a cached layer; ordering instructions well makes rebuilds fast."}', null),
  ...
  ('ml-cicd', 'Checkpoint quiz', 'quiz', 3, '{"question":"CI for ML differs from app CI because it must also validate…","options":["Only code style","Data and model quality","Commit messages","Branch names"],"correctIndex":1,"explanation":"ML behavior depends on data and weights, so pipelines test datasets and model metrics, not just code."}', null),
  ...
  ('ml-production', 'Checkpoint quiz', 'quiz', 3, '{"question":"A feature store primarily solves…","options":["GPU scheduling","Consistent features between training and serving","Model compression","Data visualization"],"correctIndex":1,"explanation":"Training/serving skew disappears when both read identical feature definitions from one store."}', null),
  ('ml-platform', 'Work through the MLOps Zoomcamp capstone material', 'watch', 1, null, null),
  ('ml-platform', 'Build: an end-to-end platform design doc — tracking, registry, deploy paths', 'build', 2, null, null),
  ('ml-platform', 'Checkpoint quiz', 'quiz', 3, '{"question":"A model registry provides…","options":["Faster training","Versioned, stage-managed models with lineage","Data labeling","Feature computation"],"correctIndex":1,"explanation":"Registries track model versions, stages (staging/prod), and lineage — the backbone of controlled deployment."}', null)
